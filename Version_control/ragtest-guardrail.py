...
class RAGConfig:
    ...
    def __init__(self, config_path="config.ini"):
        ...
        self.defaults = {
            ...
            # Sensitive data config could go here too, but let's define in code for now
        }
...
class RAGSystem:
    ...
    def __init__(self, config_path="config.ini"):
        ...
        # === START: Guardrail Definitions ===
        self.sensitive_keywords = [
            "Project Phoenix", "Project Bluebird", "Client Confidential Inc",
            "Internal Code Alpha", "Salary", "Employee SSN",
            # Add more specific keywords or phrases here
        ]
        self.sensitive_patterns = [
            re.compile(r'\b\d{3}-\d{2}-\d{4}\b'),  # SSN Format xxx-xx-xxxx
            re.compile(r'\b\d{4}-\d{4}-\d{4}-\d{4}\b'), # Basic Credit Card format xxxx-xxxx-xxxx-xxxx
            re.compile(r'\b\d{4} \d{4} \d{4} \d{4}\b'), # Basic Credit Card format xxxx xxxx xxxx xxxx
            re.compile(r'(\$[0-9,]{5,}\.[0-9]{2})'), # Example: Large dollar amounts like $10,000.00 or more
            re.compile(r'\b(password|secret|passwd)\b', re.IGNORECASE), # Common secret words
            # Add more complex regex patterns here
        ]
        self.redaction_placeholder = "[REDACTED]"
        logger.info(f"Guardrails initialized with {len(self.sensitive_keywords)} keywords and {len(self.sensitive_patterns)} patterns.")
        # === END: Guardrail Definitions ===
    ...
    # (_extract_pdf, _extract_xlsx, _extract_csv, _extract_pptx - unchanged)
    ...
    # === START: Guardrail Application Function ===
    def _apply_guardrails(self, text):
        """Redacts sensitive information from text based on predefined keywords and patterns."""
        ...
        # Apply keyword redaction (case-insensitive, whole word)
        for keyword in self.sensitive_keywords:
            # Use regex to match whole word, case-insensitive
            pattern = re.compile(r'\b' + re.escape(keyword) + r'\b', re.IGNORECASE)
            new_text, count = pattern.subn(self.redaction_placeholder, redacted_text)
            if count > 0:
                redacted_text = new_text
                redaction_applied = True
                logger.debug(f"Guardrail: Redacted keyword '{keyword}' {count} time(s).")
        ...
        for pattern in self.sensitive_patterns:
            ...
            if count > 0:
                redacted_text = new_text
                redaction_applied = True
                logger.debug(f"Guardrail: Redacted pattern '{pattern.pattern}' {count} time(s).")

        if redaction_applied:
            logger.warning("Guardrails applied: Sensitive content was redacted from the output.")
        return redacted_text
    # === END: Guardrail Application Function ===


    def query_llm(self, query, context_data, retry_count=1):
        """Query LLM with aggregated text context. Applies OUTPUT guardrails."""
        ...
        # --- Optional: Apply guardrails to CONTEXT before sending to LLM ---
        # combined_context = self._apply_guardrails(combined_context)
        # if "[REDACTED]" in combined_context:
        #     logger.warning("Guardrail: Sensitive information redacted from context before sending to LLM.")
        # --- End Optional Context Guardrail ---
        ...
        try:
            ...
            raw_answer = f"Error: LLM query failed after retries."
            for attempt in range(retry_count + 1):
                try:
                    ...
                    logger.info(f"Groq response received (attempt {attempt+1}). Length: {len(raw_answer)}")
                    if not raw_answer and attempt < retry_count: logger.warning("Empty response, retrying..."); time.sleep(1); continue
                    # Apply guardrails to the final answer before returning
                    final_answer = self._apply_guardrails(raw_answer or f"Received empty response from LLM for text context from {source_file_str}.")
                    return final_answer
                    ...
                    else: raw_answer = f"Error: Failed to get answer from LLM after {retry_count+1} attempts. (API Error: {e_api})"

            # Apply guardrails even if retries failed with an error message
            final_answer = self._apply_guardrails(raw_answer)
            return final_answer
            ...
            # Apply guardrails to the error message itself
            return self._apply_guardrails(f"Error: Could not query LLM due to setup error: {e_setup}")


    # ==========================================================================
    # START OF NEW/MODIFIED HELPER METHODS FOR QUERY ANALYSIS & DATAFRAME OPS
    # ==========================================================================
    # (_call_llm_for_analysis, _classify_query, _decompose_query, _generate_pandas_code unchanged)
...
"""
    ...
    def _decompose_query(self, query):
        ...
        # (Unchanged)
        system_prompt = """You are an expert query decomposer. Your task is to break down a complex user query that requires combining information from TEXT passages into a series of simpler, factual sub-queries. Each sub-query should aim to retrieve a specific piece of information from text needed to answer the original query.
...
Format the output as a numbered list, with each sub-query on a new line. If the query cannot be decomposed or is already simple, return the original query prefixed with '1. '."""
    ...
    def _generate_pandas_code(self, query, df_columns, df_name="df"):
        ...
        # (Unchanged - Prompt already tries to limit code generation)
        # Note: Guardrails are applied to the *output* of the executed code, not the code itself.
...
"""
    ...
    # ==========================================================================
    # START OF MODIFIED run_query FUNCTION (integrating Guardrails)
    # ==========================================================================
    def run_query(self, query, context_strategy="top_k"):
        """Complete query pipeline: classify, retrieve, (DataFrame query OR text RAG) with Guardrails."""
        logger.info(f"--- Starting query execution (v4 + Guardrails): '{query[:100]}...' ---")
        final_results = {
            "query": query,
            "classification": None,
            ...
            "dataframe_result_raw": None, # Raw result BEFORE guardrails
            "dataframe_result_redacted": None, # Result AFTER guardrails
            "aggregated_context_data": {}, # For text RAG path
            "answer": "", # Final, potentially redacted answer
            "answer_source": None, # 'DataFrame' or 'Text RAG'
            "status": "Started",
            "error": None,
            "guardrails_applied": False # Track if redaction occurred
        }
        ...
        try:
            ...
            if classification == "Structured Query (DataFrame)" and confidence >= self.config.dataframe_query_confidence_threshold and self.dataframes:
                ...
                if text_retrieval_results:
                    best_score = float('inf'); best_file = None
                ...
                if target_df_name:
                    ...
                    if pandas_code:
                        ...
                        # Define execution scope outside try/except for clarity
                            ...
                            with st.spinner(spinner_msg):
                                try:
                                    with redirect_stdout(string_io):
                                        # !!! SECURITY RISK !!! Still present - Guardrails affect OUTPUT only.
                                        exec(pandas_code, {'pd': pd, 'np': np}, local_scope)
                                    raw_result_value = string_io.getvalue().strip()
                        ...
                        if execution_successful:
                            logger.info(f"Pandas code execution successful. Raw result length: {len(raw_result_value)}")
                            final_results["dataframe_result_raw"] = raw_result_value

                            # *** APPLY GUARDRAILS TO DATAFRAME RESULT ***
                            redacted_result_value = self._apply_guardrails(raw_result_value)
                            final_results["dataframe_result_redacted"] = redacted_result_value
                            if raw_result_value != redacted_result_value:
                                final_results["guardrails_applied"] = True

                            # Use the REDACTED result for the final answer
                            final_results["answer"] = f"Based on analysis of the structured data in '{target_df_name}':\n\n```\n{redacted_result_value}\n```"
                            final_results["status"] = "Completed Successfully"
                            final_results["answer_source"] = "DataFrame"
                            dataframe_succeeded = True
                            final_results["error"] = None # Clear any previous error like 'no DF found' if execution succeeded
                    else:
            ...
            if not dataframe_succeeded:
                 ...
                 if not final_results.get("retrieval_results"):
                     ...
                     final_results["answer"] = self._apply_guardrails("Could not find relevant information in the documents.") # Apply guardrails even to canned responses
                     if final_results["error"]: # Keep previous error if DF path failed first
                        final_results["answer"] += f" (Note: {self._apply_guardrails(final_results['error'])})" # Guardrail error message too
                 ...
                 if not aggregated_context_data or not aggregated_context_data.get("combined_context"):
                      ...
                      final_results["answer"] = self._apply_guardrails("Could not prepare text context for answering.")
                      prev_error = final_results.get("error") # Preserve earlier error
                      final_results["error"] = "Text context aggregation failed or empty."
                      if prev_error: final_results["error"] += f" (Previous error: {self._apply_guardrails(prev_error)})" # Guardrail previous error
                 ...
                 # Note: query_llm now handles output guardrails internally
                 ...
                 final_results["answer"] = answer # Answer is already redacted by query_llm
                 # Check if query_llm applied redactions (by comparing with raw before redaction, if we stored it, or checking for placeholder)
                 if self.redaction_placeholder in answer:
                     final_results["guardrails_applied"] = True
                 final_results["status"] = "Completed Successfully"
                 # Keep error if it came from a failed DF attempt before fallback, apply guardrails to it
                 if dataframe_attempted and not dataframe_succeeded and final_results.get("error"):
                    ...
                    final_results["error"] = self._apply_guardrails(final_results["error"]) # Apply guardrails to the preserved error message
                 elif not final_results.get("error"): # Clear error if text RAG succeeded cleanly
                     final_results["error"] = None

            # Final Logging
            logger.info(f"--- Finished query execution (v4 + Guardrails) for: '{query[:100]}...'. Answer Source: {final_results.get('answer_source')}. Guardrails applied: {final_results['guardrails_applied']} ---")

        except Exception as e:
            logger.error(f"Unexpected error during run_query (v4 + Guardrails): {e}", exc_info=True)
            final_results["status"] = "Failed";
            redacted_error = self._apply_guardrails(str(e)) # Apply guardrails to the exception message
            final_results["error"] = redacted_error
            final_results["answer"] = f"An unexpected error occurred: {redacted_error}"
            final_results["guardrails_applied"] = str(e) != redacted_error # Check if error message was redacted
            if is_streamlit: st.error(f"An unexpected error occurred: {redacted_error}")
...
# (Streamlit UI code MODIFIED to indicate guardrails)
if __name__ == "__main__":
    ...
    st.title("📄 Multi-Document Question Answering System (v4 + Guardrails)")
    st.caption("Query PDFs, PPTX (Text RAG) & XLSX, CSV (DataFrame Query / Text RAG Fallback) using Embeddings, FAISS, Pandas, Groq LLM")
    st.info("🛡️ Content Guardrails are active. Sensitive information patterns may be redacted.")
    ...
    try:
        ...
        if is_ready and processed_files_list:
            ...
            # Indicate Guardrails are active in sidebar too
            st.sidebar.info(f"Guardrails Active: Yes")
            with st.sidebar.expander("Guardrail Keywords/Patterns"):
                st.caption("**Keywords (Case-Insensitive):**")
                for kw in rag_sys.sensitive_keywords: st.code(kw, language=None)
                st.caption("**Regex Patterns:**")
                for p in rag_sys.sensitive_patterns: st.code(p.pattern, language='regex')
            ...
            if user_query and st.button("Get Answer", key="submit_query"):
                ...
                # Show warning if guardrails were applied
                if results_data.get("guardrails_applied"):
                    st.warning(f"🛡️ Note: Content guardrails were applied. Some information may have been redacted as '{rag_sys.redaction_placeholder}'.")

                if answer: st.markdown(answer) # Display potentially redacted answer
                ...
                with col1:
                    ...
                    st.caption(f"Confidence: `{results_data.get('classification_confidence', 0.0):.2f}`") # Default to 0.0 if None
                ...
                with col2:
                     ...
                     st.caption(f"Guardrails Applied: {'Yes' if results_data.get('guardrails_applied') else 'No'}") # Show guardrail status
                ...
                if results_data.get("answer_source") == "DataFrame" and results_data.get("target_dataframe"):
                     ...
                     # Show redacted result if different from final formatted answer
                     if results_data.get("dataframe_result_redacted") and f"```{results_data['dataframe_result_redacted']}```" not in answer:
                          with st.expander("Redacted DataFrame Result (Direct Output)"):
                              st.text(results_data["dataframe_result_redacted"])
                ...
                if combined_context and results_data.get("answer_source") == "Text RAG": # Only show if Text RAG was the source
                     # Note: This context is BEFORE it goes to the LLM, so it might contain sensitive info
                     # Consider applying guardrails here too if displaying, or add a warning
                     with st.expander(f"Text Context Used from: {', '.join(context_source_files)}"):
                         st.caption("⚠️ This is the raw context retrieved *before* being sent to the LLM and *before* final answer guardrails.")
                         # Optionally apply guardrails just for display:
                         # display_context = rag_sys._apply_guardrails(combined_context)
                         # st.text_area("Combined Text Context Sent to LLM", display_context, height=250, key="context_display")
                         # Or show raw with warning:
                         st.text_area("Combined Text Context Sent to LLM (Potentially Unredacted)", combined_context, height=250, key="context_display")
                ...
                # Show Text Retrieval Results
                retrieval_results = results_data.get("retrieval_results", {})
                if retrieval_results:
                     # Note: Chunk content might contain sensitive info. Apply guardrails for display?
                     with st.expander("Retrieved Text Chunks (Top candidates used for context/DataFrame identification)"):
                         ...
                         st.caption("⚠️ Chunk content displayed below is raw and may contain unredacted sensitive information.")
                         for i, res in enumerate(display_chunks[:rag_sys.config.k_retrieval * 2]): # Show more for inspection
                             ...
                             # Apply guardrails *just for display* if desired:
                             # display_content = rag_sys._apply_guardrails(res['content'])
                             # st.text(f"{display_content[:350]}...")
                             # Or show raw with warning (as currently implemented):
        ...
        else: # is_ready is False
             st.error("System processed files but is not marked as ready. Check logs.")

    except Exception as e:
        # Apply guardrails to the error message in the main exception handler too
        redacted_e = rag_sys._apply_guardrails(str(e)) if 'rag_sys' in locals() else str(e)
        st.error(f"Streamlit App Error: {redacted_e}")
...
# --- END OF FILE ragtest_c2-4.py ---
# --- END OF FILE ragtest_c4_dataframe.py ---
