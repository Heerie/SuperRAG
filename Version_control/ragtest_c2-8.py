...
class RAGSystem:
                    ...
                    try:
                        for table in page.extract_tables():
                             if table:
                                 ...
                                 if not pd.api.types.is_numeric_dtype(table_df.iloc[0].dropna()):
                                     try:
                                 ...
                                 if cleaned_table_string:
                                     all_content.append({
                                        ...
                                        "source_info": source_info, # Use same page source info
                                        "file_type": "pdf",
    ...
    def _infer_sql_type(self, series):
        ...
        if num_total > 0 and num_numeric / num_total > 0.9: # If >90% are numeric
            if pd.api.types.is_integer_dtype(pd.to_numeric(series, errors='coerce')): return "INTEGER"
            if pd.api.types.is_float_dtype(pd.to_numeric(series, errors='coerce')): return "REAL"
            ...
            try:
                ...
                for expected_col_name in column_names:
                    ...
                    if expected_col_name in metadata_dict:
                        ...
                        for resp_col_name, meta in metadata_dict.items():
                             if resp_col_name.lower() == expected_col_name.lower():
                                found_meta = meta; break # Found case-insensitive match
                ...
                if missing_cols:
                     ...
                     metadata_map = {m['name']: m for m in formatted_metadata}
                     for col_name in column_names:
                         if col_name in metadata_map:
                             final_metadata_with_missing.append(metadata_map[col_name])
                         else: # Add basic entry for missing column
                             final_metadata_with_missing.append({"name": col_name, "description": "N/A - Metadata generation failed.", "type": self._infer_sql_type(df[col_name])})
                     formatted_metadata = final_metadata_with_missing # Replace with ordered list including fallbacks
                ...
                if sql_type in ["INTEGER", "REAL"]:
                    ...
                    if sql_type == "INTEGER":
                        # Attempt conversion to nullable integer type if possible
                         try: temp_df[col] = temp_df[col].astype('Int64') # Pandas nullable integer
                         except Exception: pass # Keep as float if conversion fails (e.g., contains NaNs)
                    ...
                    if chunks:
                        ...
                        if regenerate_embeddings or faiss_index is None:
                            ...
                            if not content_list: logger.warning(f"No text content to embed for {file_name}.");
                                ...
                                embeddings = self.encoder_model.encode(content_list, batch_size=32, show_progress_bar=False, convert_to_numpy=True).astype('float32') # Reduced batch size?
...
The context below comes from document(s): '{source_file_str}'. Context sections are marked with 'Source:', 'Page:', and 'Section:' (e.g., 'Section: title_page', 'Section: main_content').
                ...
                if target_table_name and target_table_name in self.table_metadata:
                    ...
                    if sql_query:
                        ...
                        if sql_error:
                            ...
                            is_fallback_answer = "had trouble summarizing it naturally" in final_answer or "synthesis failed" in final_answer
                 ...
                 if needs_fresh_retrieval:
                     ...
                     for file, res_list in all_query_results.items():
                          ...
                          for res in res_list:
                              ...
                              key = (res['content'], source_key, res.get('potential_section'))
...
# --- START OF STREAMLIT UI CODE (Modified for Chat Interface - Largely Unchanged) ---
if __name__ == "__main__":
    ...
    # Add this right after st.set_page_config in your if __name__ == "__main__": block
    st.markdown("""
    <style>
        ...
        :root {
            ...
            --primary-color-rgb: 74, 144, 226; /* Fallback if JS doesn't run */
        }
        ...
        .st-emotion-cache-16txtl3 .st-emotion-cache-1cypcdb { /* Sidebar title */
        ...
        .st-emotion-cache-16txtl3 .stAlert[data-testid="stNotificationContentWarning"] {
            background-color: rgba(var(--warning-color-rgb, 245, 166, 35), 0.1);
        ...
        .st-emotion-cache-16txtl3 .stAlert[data-testid="stNotificationContentError"] {
            background-color: rgba(var(--error-color-rgb, 208, 2, 27), 0.1);
            border-left: 4px solid var(--error-color); color: var(--error-color);
        }
        .st-emotion-cache-16txtl3 .stInfo {
        ...
        .stChatInputContainer {
            ...
            padding: 1rem 1.5rem; margin: 0 -1rem -2rem -1rem; /* Extend to edges */
        }
        ...
        .stChatMessage {
            ...
            max-width: 80%; /* Default max width for assistant */
        }
        ...
        .message-footer {
            ...
            margin-top: 0.8em;
            padding-top: 0.5em;
            border-top: 1px dashed rgba(var(--text-muted-color-rgb, 160, 160, 160), 0.3);
        }
        ...
        .stExpander summary:hover { background-color: #2a2a2a; }
        .stExpander > div { padding: 0.5rem 1rem 1rem 1rem; } /* Content padding */
        .stExpander .stCaption { text-align: left; margin-bottom: 0.3em; font-size: 0.85em;}
        .stExpander .stCodeBlock, .stExpander pre { background-color: #111 !important; }
        ...
        div[data-testid="stModal"] > div {
            ...
            max-width: 700px; /* Control dialog width */
        }
        div[data-testid="stModal"] h2 { color: var(--primary-color); margin-bottom: 1rem; }
        div[data-testid="stModal"] .stButton button {
            background-color: var(--primary-color); color: white;
            border-radius: 8px; transition: background-color 0.3s ease;
        }
        div[data-testid="stModal"] .stButton button:hover { background-color: #357ABD; }
        div[data-testid="stModal"] .stDataFrame { margin-top: 1rem; }
        div[data-testid="stModal"] .stProgress { margin-top: 0.5rem; }
        div[data-testid="stModal"] .stAlert { margin-top: 0.5rem; }
    ...
    <script>
        function setRGBColorVar(cssVarName, rgbVarName) {
            ...
            if (colorVal) {
                ...
                if (r !== undefined) { document.documentElement.style.setProperty(rgbVarName, `${r}, ${g}, ${b}`);}
            }
        }
    ...
    st.caption("Query Project Proposals (PDFs/PPTX) & Supporting Data (XLSX/CSV) with Chat History (v7-PDFStruct)")

    @st.cache_resource # Cache the RAG system instance
    def load_rag_system(config_path="config.ini"):
        # API Key check before heavy loading
        ...
        # Temp load config to check DeepSeek key for early warning
        # This avoids loading embedding model if DeepSeek is crucial and missing (optional)
        # For now, we'll just warn after full load.
        # temp_config = RAGConfig(config_path)
        # deepseek_key = temp_config.deepseek_api_key 

        system = None
        # Use a more prominent spinner for the initial heavy load
        with st.spinner("Initializing RAG System & loading embedding model... This might take a moment."):
            try:
                ...
                # This error will be caught by the main try-except in __main__ if it's fatal for Streamlit
                logger.error(f"Fatal RAG Initialization Error during RAGSystem instantiation: {e}", exc_info=True)
                # St.error here might be too early if it's just a model load fail but system partially inits
                # The main __main__ will handle st.error and st.stop
                raise # Re-raise to be caught by the main app's try-except
        
        # Check system and db_conn after instantiation attempt
        if not system or not system.db_conn:
            st.error("🚨 RAG System or SQLite connection failed during initialization! Check logs for details."); 
            st.stop()
        
        # Check DeepSeek key after successful RAGSystem load
        if not system.deepseek_api_key: # Access via system instance
    ...
    def display_processing_dialog(_rag_system):
        # Using session state to control dialog visibility for better state management
        dialog_key = "doc_management_dialog"
        if f"{dialog_key}_open" not in st.session_state:
            st.session_state[f"{dialog_key}_open"] = False

        if st.sidebar.button("📂 Manage Documents", use_container_width=True, key="open_doc_dialog"):
            st.session_state[f"{dialog_key}_open"] = True

        if st.session_state.get(f"{dialog_key}_open", False):
            with st.dialog("Document Management & Processing"): # Use st.dialog
                ...
                status_placeholder_dialog = st.empty() # Placeholder within the dialog

                def dialog_progress_callback(message, is_error=False, is_warning=False, is_done=False, current_step=None, total_steps=None, stage=None):
                    ...
                    try:
                        if 0 < progress_val < 1.0 and not is_error and not is_warning:
                            status_placeholder_dialog.progress(progress_val, text=full_message)
                        else:
                            if is_error: status_placeholder_dialog.error(full_message)
                            elif is_warning: status_placeholder_dialog.warning(full_message)
                            elif is_done: status_placeholder_dialog.success(full_message)
                            else: status_placeholder_dialog.info(full_message)
                    except Exception as e: logger.warning(f"Streamlit UI update error in dialog callback: {e}")

                if "dialog_processed_files_list" not in st.session_state: st.session_state.dialog_processed_files_list = []
                if "dialog_processing_done" not in st.session_state: st.session_state.dialog_processing_done = False
                if "dialog_processing_status_message" not in st.session_state: st.session_state.dialog_processing_status_message = "Click 'Process/Re-process Documents' to load or update data."

                if st.button("🔄 Process/Re-process Documents", key="process_docs_dialog", use_container_width=True):
                    st.session_state.dialog_processing_done = False
                    st.session_state.dialog_processed_files_list = []
                    st.session_state.dialog_processing_status_message = "Starting processing..."
                    status_placeholder_dialog.info(st.session_state.dialog_processing_status_message)
                    
                    # This spinner will appear inside the dialog
                    with st.spinner("Processing documents... This may take a while."):
                        try:
                            processing_successful = _rag_system.process_files(progress_callback=dialog_progress_callback)
                            st.session_state.dialog_processed_files_list = sorted(list(_rag_system.processed_files))
                            st.session_state.dialog_processing_done = True
                            text_idx_count = len(_rag_system.faiss_indexes); sql_tbl_count = len(_rag_system.table_metadata)
                            processed_count = len(st.session_state.dialog_processed_files_list)
                            final_msg = f"Processed {processed_count} file(s). Text Indices: {text_idx_count}, SQL Tables: {sql_tbl_count}."
                            
                            if processed_count > 0: st.session_state.dialog_processing_status_message = f"✅ Ready! {final_msg}"; status_placeholder_dialog.success(st.session_state.dialog_processing_status_message)
                            elif processing_successful: st.session_state.dialog_processing_status_message = f"⚠️ Finished, but no supported documents processed. {final_msg}"; status_placeholder_dialog.warning(st.session_state.dialog_processing_status_message)
                            else: st.session_state.dialog_processing_status_message = f"❌ Processing function reported failure."; status_placeholder_dialog.error(st.session_state.dialog_processing_status_message)
                            
                            # Update global readiness state if processing was done from dialog
                            st.session_state.is_ready_for_chat = st.session_state.dialog_processing_done and len(st.session_state.dialog_processed_files_list) > 0
                            # No st.rerun() here, dialog handles its own state. Closing it will refresh main.
                        except Exception as e:
                            logger.error(f"Fatal error during document processing (dialog): {e}", exc_info=True)
                            st.session_state.dialog_processing_status_message = f"❌ Fatal error: {e}."
                            status_placeholder_dialog.error(st.session_state.dialog_processing_status_message)
                            st.session_state.dialog_processing_done = False
                            st.session_state.dialog_processed_files_list = []
                            st.session_state.is_ready_for_chat = False
                else:
                    # Display stored status message for the dialog
                    current_msg = st.session_state.dialog_processing_status_message
                    if "✅ Ready!" in current_msg: status_placeholder_dialog.success(current_msg)
                    elif "❌" in current_msg: status_placeholder_dialog.error(current_msg)
                    elif "⚠️" in current_msg: status_placeholder_dialog.warning(current_msg)
                    else: status_placeholder_dialog.info(current_msg)

                if st.session_state.dialog_processed_files_list:
                    st.markdown("--- \n**Loaded Assets:**")
                    assets_df_data = []
                    for fname in st.session_state.dialog_processed_files_list:
                        asset_types = []
                        ...
                        if fname in _rag_system.file_to_table_map and _rag_system.file_to_table_map.get(fname):
                            ...
                            asset_types.append(f"SQL ({len(tables)} table{'s' if len(tables) > 1 else ''})")
                ...
                if st.button("Close", key="close_doc_dialog", use_container_width=True):
                    st.session_state[f"{dialog_key}_open"] = False
                    st.rerun() # Rerun main page to reflect any changes (e.g., is_ready_for_chat)

        # Return readiness based on a global session state variable, updated by dialog processing
    ...
    #     try:
    #         # Add a spinner during system init as embedding models can take time
    ...
    # --- Document Processing UI (Moved to Sidebar/Expander - Unchanged) ---
    def display_processing_ui(_rag_system):
        with st.sidebar.expander("🔄 Document Management", expanded=False):
            st.caption(f"Data Folder: `{os.path.abspath(_rag_system.config.data_dir)}`")
            st.caption(f"Index Folder: `{os.path.abspath(_rag_system.config.index_dir)}`")
            st.caption(f"DB Location: `{_rag_system.config.sqlite_db_path}`")
            ...
            def streamlit_progress_callback(message, is_error=False, is_warning=False, is_done=False, current_step=None, total_steps=None, stage=None):
                ...
                progress_val = 1.0 if is_done else 0.0
                if total_steps and current_step is not None:
                     # Simple progress estimation based on stage
                     stage_prog = {"Extracting Text": 0.1, "Chunking Text": 0.15, "Verifying Index": 0.2, "Embedding Text": 0.3, "Indexing Text": 0.4,
                                   "Loading Sheet": 0.5, "Analyzing Columns": 0.7, "Loading SQL": 0.9}.get(stage, 0.0)
                     progress_val = min(1.0, (current_step + stage_prog) / total_steps) if total_steps > 0 else 0.0
                ...
                # status_messages.append(full_message) # Append causes list growth on reruns
                # if len(status_messages) > 10: status_messages.pop(0)
                # Instead, directly update the placeholder
                try:
                    # Display progress bar only during active processing stages
                    if 0 < progress_val < 1.0 and not is_error and not is_warning:
                        status_placeholder.progress(progress_val, text=full_message) # Show message with progress
                    else:
                        # Clear progress bar when done, failed, or warning, show only caption/status
                        if is_error: status_placeholder.error(full_message)
                        elif is_warning: status_placeholder.warning(full_message)
                        else: status_placeholder.caption(full_message) # Use caption for non-error final/idle states
                except Exception as e:
            ...
            if st.button("Process/Re-process Documents", key="process_docs"):
                 ...
                 st.session_state.processed_files_list = []
                 st.session_state.processing_status_message = "Starting processing..."
                 status_placeholder.info(st.session_state.processing_status_message) # Initial message
                 with st.spinner("Processing documents... This may take a while."):
                     try:
                         ...
                         if processed_file_count > 0:
                             ...
                             status_placeholder.success(st.session_state.processing_status_message)
                         elif processing_successful: # Finished but loaded nothing
                             st.session_state.processing_status_message = f"⚠️ Finished, but no supported documents were successfully processed. {final_message}"
                             status_placeholder.warning(st.session_state.processing_status_message)
                         else: # process_files returned False (shouldn't happen often)
                             st.session_state.processing_status_message = f"❌ Processing function reported failure. Check logs."
                             status_placeholder.error(st.session_state.processing_status_message)

                         st.rerun() # Rerun to update the main UI state check based on processing_done
                     except Exception as e:
                         logger.error(f"Fatal error during document processing call: {e}", exc_info=True)
                         st.session_state.processing_status_message = f"❌ Fatal error during processing: {e}. Check logs."
                         status_placeholder.error(st.session_state.processing_status_message) # Show error permanently
                         st.session_state.processing_done = False
                         st.session_state.processed_files_list = []
            else:
                 # Display the stored status message on subsequent runs if not actively processing
                 status_level = "info"
                 current_msg = st.session_state.processing_status_message
                 if "✅ Ready!" in current_msg: status_level = "success"
                 elif "❌" in current_msg: status_level = "error"
                 elif "⚠️" in current_msg: status_level = "warning"

                 if status_level == "success": status_placeholder.success(current_msg)
                 elif status_level == "error": status_placeholder.error(current_msg)
                 elif status_level == "warning": status_placeholder.warning(current_msg)
                 else: status_placeholder.info(current_msg) # Default or initial message


            # Display processed files if available
            if st.session_state.processed_files_list:
                 st.caption("Loaded Assets:")
                 assets_df_data = []
                 for fname in st.session_state.processed_files_list:
                     ...
                     assets_df_data.append({"File": fname, "Type(s)": ", ".join(asset_types)})

                 if assets_df_data:
                    st.dataframe(pd.DataFrame(assets_df_data), use_container_width=True, hide_index=True)

        # Return True only if processing completed and at least one file was processed
    ...
    try:
        ...
        # Call the function to display the "Manage Documents" button and handle dialog
        # The function now returns the readiness state.
        # Initialize is_ready_for_chat if it doesn't exist
        if "is_ready_for_chat" not in st.session_state:
            # Check if initial processing might have already occurred (e.g. from a previous run without dialog)
            # A simple check: if rag_sys has processed_files, assume it's ready.
            # This is a basic heuristic. A more robust way would be to always force processing via dialog on first run.
            if rag_sys and rag_sys.processed_files:
                st.session_state.is_ready_for_chat = True
                if "dialog_processed_files_list" not in st.session_state: # Sync dialog state
                    st.session_state.dialog_processed_files_list = sorted(list(rag_sys.processed_files))
                if "dialog_processing_status_message" not in st.session_state:
                    st.session_state.dialog_processing_status_message = f"✅ Ready! {len(rag_sys.processed_files)} file(s) previously processed."

            else:
                st.session_state.is_ready_for_chat = False


        is_ready = display_processing_dialog(rag_sys) # This handles the dialog logic
        ...
        # --- Main Chat Area ---
        if not is_ready:
             st.info("⬅️ Please process documents using 'Manage Documents' in the sidebar to enable chat.")
             st.stop()
        
        chat_container = st.container() # Use a container for chat messages for potential height restrictions

        with chat_container:
            for i, message in enumerate(st.session_state.messages):
                avatar_icon = "👤" if message["role"] == "user" else "🛰️"
                # The CSS targets [avatar_style="user"] or [avatar_style="assistant"]
                # Streamlit's st.chat_message automatically sets this based on the role.
                with st.chat_message(message["role"], avatar=avatar_icon): # CSS will style based on role
                    
                    # Separate content from footer for styling
                    content_part = message["content"]
                    footer_part = ""
                    if "\n\n---\n*" in message["content"] and message["role"] == "assistant":
                    ...
                    # Show analysis details for the *last* assistant message
                    if message["role"] == "assistant" and i == len(st.session_state.messages) - 1 and st.session_state.get('analysis_details', {}).get('has_details'):
                        with st.expander("Show Analysis & Context", expanded=False):
                            ...
                            if details.get("Text Sources Used"): st.caption(f"**Text Sources Used:** {', '.join(details['Text Sources Used'])}")
        ...
        if prompt := st.chat_input("Ask a question about the SatSure proposals..."):
            ...
            # For user messages, we don't need a complex footer from RAG, so direct display is fine
            # CSS will handle user message styling.
            
            # No immediate display for user, will be handled by rerun at the end of this block

            with st.spinner("🛰️ SatSure AI is thinking..."): # Spinner for assistant response
                ...
                query_end_time = time.time() # Assuming query_start_time was captured if needed
                # elapsed_time = query_end_time - query_start_time # Not explicitly set here, but was in original
                ...
                elapsed_time_str = f"{time.time() - st.session_state.get('query_start_time', time.time()):.2f}" # Get start time if set
                
                full_response = answer
                response_footer_content = f"Answer generated via {answer_source} in {elapsed_time_str}s."
                if "Completed Successfully" not in query_status:
                    if query_status != "Completed Successfully": response_footer_content += f"\nStatus: {query_status}"
                    if query_error and query_error not in full_response: response_footer_content += f"\nNote: {query_error}"
                
                if answer.strip() != "I don't really know but my next versions will be able to answer this for sure!":
            ...
            st.rerun() # Rerun to display new user message and assistant response + analysis expander
        ...
        try:
            if 'rag_sys' in locals() and hasattr(rag_sys, 'config'): log_file_path = rag_sys.config.log_file
        except Exception: pass
