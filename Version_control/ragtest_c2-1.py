# --- START OF FILE ragtest_c2.py ---

# --- START OF FILE ragtest_c.py --- # <-- This comment remains from the original, keep as is.
    ...
    def __init__(self, config_path="config.ini"):
        ...
        self.defaults = {
            ...
            "MODELS": {
                ...
                "llm_model": "meta-llama/llama-4-scout-17b-16e-instruct", # User specified model name, check if available on Groq if using
                "device": "auto"
            },
            "PARAMETERS": {
                "chunk_size": "200",
...
class RAGSystem:
    ...
    def __init__(self, config_path="config.ini"):
        ...
        try:
            # Check if running in Streamlit context before showing spinner
            ...
            else: # Load normally if not in Streamlit
            ...
            if "streamlit" in sys.modules:
                ...
                raise # Re-raise the exception if not in Streamlit
    ...
    def _extract_pdf(self, file_path):
        ...
        try:
            with pdfplumber.open(file_path) as pdf:
                ...
                page_bar = None
                # Check if running in Streamlit before using st.progress
                if "streamlit" in sys.modules and total_pages > 10:
    ...
    def _extract_xlsx(self, file_path):
        ...
        try:
            ...
            if "streamlit" in sys.modules and total_sheets > 1: # Show bar only if multiple sheets
            ...
            for i, sheet_name in enumerate(excel_file.sheet_names):
                try:
                    ...
                    # Simple approach: Convert entire sheet to a string representation
                    sheet_string = df.to_string(index=False, header=True, na_rep='')

                    cleaned_content = self.clean_text(sheet_string)
                    if cleaned_content:
                        # Truncate if the whole sheet as string is excessively long
                        if len(cleaned_content) > max_chars * 10: # Example limit
    ...
    def _extract_csv(self, file_path):
        ...
        try:
            ...
            for enc in encodings_to_try:
                 try:
                     # Use low_memory=False to potentially help with mixed types
    ...
    # ==========================================================================
    # START OF MODIFIED _extract_pptx FUNCTION
    # ==========================================================================
    def _extract_pptx(self, file_path):
        """Extract text from PPTX files, prepending the previous slide's title if available."""
        all_content = []
        base_filename = os.path.basename(file_path)
        max_chars = self.config.max_chars_per_element
        try:
            ...
            # Check if running in Streamlit before using st.progress
            if "streamlit" in sys.modules and total_slides > 1:
                 slide_bar = st.progress(0, text=f"Extracting slides from {base_filename}...")

            previous_slide_title_text = None # Store the cleaned title text of the previous slide

            for i, slide in enumerate(prs.slides):
                current_slide_main_texts = []
                current_slide_title_text = "" # Store the cleaned title of *this* slide for the *next* iteration

                # --- Try to get the title of the CURRENT slide ---
                # Check if the title placeholder exists and has text
                title_shape = None
                try:
                    title_shape = slide.shapes.title
                except AttributeError:
                    logger.debug(f"Slide {i+1} in {base_filename} has no designated title placeholder.")
                    title_shape = None

                if title_shape and title_shape.has_text_frame:
                    raw_title_text = title_shape.text_frame.text
                    cleaned_title = self.clean_text(raw_title_text)
                    if cleaned_title:
                        current_slide_title_text = cleaned_title
                        # We capture it here but don't add it to current_slide_main_texts *yet*,
                        # it will be prepended to the *next* slide's content.
                        # However, we *could* add it here too if we want the title slide to *also* exist independently.
                        # Let's add it for completeness, marked clearly.
                        current_slide_main_texts.append(f"[Slide Title Only]: {cleaned_title}")


                # --- Extract text from OTHER shapes on the current slide ---
                for shape in slide.shapes:
                    # Skip the title shape if we already processed it above
                    ...
                    if shape.has_text_frame:
                        ...
                        if cleaned:
                             ...
                             current_slide_main_texts.append(cleaned)

                # --- Extract text from notes slide ---
                if slide.has_notes_slide:
                    try: # Add extra try-except for potential issues with notes frame
                        ...
                        if cleaned_notes:
                            ...
                            current_slide_main_texts.append(f"[Notes]: {cleaned_notes}")
                    except Exception as e_notes:
                        logger.warning(f"Could not extract notes from slide {i+1} in {base_filename}: {e_notes}")


                # --- Combine all text extracted *from the current slide* ---
                slide_content_from_current = "\n".join(current_slide_main_texts).strip()

                # --- Construct the FINAL content block for this slide ---
                # Start with the prepended title from the *previous* slide, if it exists
                final_slide_content_parts = []
                if previous_slide_title_text:
                    # Add a clear marker indicating the prepended title context
                    final_slide_content_parts.append(f"[Context from Title on Previous Slide ({i})]: {previous_slide_title_text}")

                # Add the main content extracted from *this* slide
                if slide_content_from_current:
                    final_slide_content_parts.append(slide_content_from_current)

                # Join the parts together
                final_slide_content = "\n\n---\n\n".join(final_slide_content_parts).strip() # Use a clear separator


                # --- Create the content block entry ---
                if final_slide_content: # Only add if there's *any* content
                    all_content.append({
                        "type": "slide_text",
                        "content": final_slide_content, # Use the potentially combined content
                        "source_info": {"slide": i + 1}, # Source metadata still refers to the *current* slide number
                        "file_type": "pptx"
                    })

                # --- Update previous_slide_title for the *next* iteration ---
                previous_slide_title_text = current_slide_title_text # Use the cleaned title we found on the current slide
            ...
            logger.info(f"Extracted {len(all_content)} content blocks (slides, with potential title prepending) from PPTX: {base_filename}")
            return all_content
        except Exception as e:
            ...
            return []
    # ==========================================================================
    # END OF MODIFIED _extract_pptx FUNCTION
    # ==========================================================================
    ...
    def extract_content(self, file_path):
        ...
        if extension == '.pdf':
            ...
            return self._extract_pptx(file_path) # Calls the modified function
        else:
    ...
    def chunk_content(self, all_content):
        ...
        # Estimate total words for progress bar
        try:
        ...
        chunk_bar = None
        # Check if running in Streamlit before using st.progress
        if "streamlit" in sys.modules and total_words_estimate > 5000:
             chunk_bar = st.progress(0, text=f"Chunking content...")

        words_processed = 0
        for item_index, item in enumerate(all_content): # Added index for logging
            content = item.get('content', '')
            source_info = item.get('source_info', {}) # Get original source info
            file_type = item.get('file_type', 'unknown') # Get file type
            content_type = item.get('type', 'unknown') # Get content block type
    ...
    def process_files(self, progress_callback=None): # Renamed from process_pdfs
        ...
        for idx, file_name in enumerate(process_list):
            ...
            try:
                ...
                if chunks is None:
                    ...
                    all_content = self.extract_content(file_path) # This now calls the modified _extract_pptx
                    if not all_content:
                ...
                if os.path.exists(index_path) and os.path.exists(emb_path) and os.path.exists(chunks_path):
                    ...
                    try:
                        ...
                        if embeddings.ndim != 2 or embeddings.shape[1] != embedding_dim:
                             ...
                             logger.warning(f"Chunk/Embedding count mismatch for {file_name} ({len(chunks)} chunks vs {embeddings.shape[0]} embeddings). Regenerating.")
                             regenerate_embeddings = True
                        else:
                             faiss_index = self.load_faiss_index(file_name, embedding_dim)
                             if faiss_index.ntotal != embeddings.shape[0]:
                                 logger.warning(f"FAISS index count doesn't match embeddings for {file_name} ({faiss_index.ntotal} vs {embeddings.shape[0]}). Rebuilding index.")
                                 faiss_index = faiss.IndexFlatL2(embedding_dim)
                                 faiss_index.add(embeddings.astype('float32'))
                                 faiss.write_index(faiss_index, index_path)
                                 logger.info(f"Rebuilt and saved FAISS index for {file_name} with {faiss_index.ntotal} vectors.")
                             else:
                ...
                if regenerate_embeddings or faiss_index is None:
                    ...
                    # Simple progress for embedding if not using built-in bar
                    embed_start_time = time.time()
                    embeddings = self.encoder_model.encode(
                        content_list, batch_size=64, show_progress_bar=False, convert_to_numpy=True
                    ).astype('float32')
                    embed_time = time.time() - embed_start_time
                    logger.info(f"Generated {embeddings.shape[0]} embeddings for {file_name} in {embed_time:.2f}s.")
                ...
                # Cleanup attempt
                try:
                    if os.path.exists(index_path): os.remove(index_path); logger.debug(f"Removed {index_path}")
                    if os.path.exists(emb_path): os.remove(emb_path); logger.debug(f"Removed {emb_path}")
                    if os.path.exists(chunks_path): os.remove(chunks_path); logger.debug(f"Removed {chunks_path}") # Remove potentially inconsistent chunks too
                    logger.info(f"Cleaned up index/embedding/chunk files for failed {file_name}")
                ...
                # Don't add to processed_files if it failed
        ...
        if final_indexed_count > 0:
             ...
             # Return True here because processing *finished*, even if nothing was indexed.
             # The calling function should check len(self.faiss_indexes).
             return True
    ...
    def query_files(self, query): # Renamed from query_pdfs
        ...
        try:
            ...
            # Check if running in Streamlit before using st.progress
            if "streamlit" in sys.modules and total_indexes > 5:
            ...
            for file_name, index in self.faiss_indexes.items():
                ...
                try:
                    ...
                    for i, idx in enumerate(indices):
                        # FAISS can return -1 if k > number of elements
                        if idx == -1: continue
                        # Basic bounds check
                        if not (0 <= idx < len(current_file_chunks)):
                             logger.warning(f"Retrieved invalid index {idx} (out of bounds {len(current_file_chunks)}) for {file_name}. Skipping.")
                             continue
                        # Avoid duplicates if FAISS returns them for some reason
                    ...
                    if file_results:
                        ...
                        logger.debug(f"Found {len(file_results)} relevant chunks in {file_name}")
                         ...
                         logger.debug(f"No relevant chunks found meeting criteria in {file_name}")
    ...
    def aggregate_context(self, query_results, strategy="top_k"):
        ...
        # Flatten results and sort globally by score (lower is better for L2)
        flat_results = []
        for file_name, results in query_results.items():
            for res in results:
                flat_results.append({**res, "file_name": file_name}) # Add filename to each result
        flat_results.sort(key=lambda x: x['score'])


        # Aggregate context from the globally best chunks across all files
        aggregated_context_str = ""
        added_chunks_count = 0
        context_sources = set() # Track which files contributed
        ...
        for res in flat_results:
            ...
            if file_type == 'pdf':
                ...
                # Include the slide number from source_info
            ...
            if total_aggregated_chars + content_chars <= max_chars:
                ...
                context_sources.add(file_name)
                 ...
                 # Try to add a truncated version if it's the very first chunk
                 if added_chunks_count == 0:
                     remaining_chars = max_chars - total_aggregated_chars - len(content_header) - len("\n\n[...TRUNCATED CONTEXT...]\n\n")
                     if remaining_chars > 50: # Need space for header and truncation marker
                         truncated_body = content_body[:remaining_chars]
                         aggregated_context_str += content_header + truncated_body + "\n[...TRUNCATED CONTEXT...]\n\n"
                         total_aggregated_chars += len(aggregated_context_str) # Recalculate total chars
                         added_chunks_count += 1
                         context_sources.add(file_name)
                         logger.warning(f"Context truncated (first chunk was too large).")
                     else:
                         logger.warning(f"First relevant chunk is too large ({content_chars} chars) to fit within the {max_chars} char limit, skipping.")
                 ...
                 break # Exit the loop once limit is hit

        final_context = aggregated_context_str.strip()
        if final_context:
             # Return a single context string and the list of source files used
             all_context = {
                 "combined_context": final_context,
                 "source_files": sorted(list(context_sources))
             }
             logger.info(f"Aggregated {total_aggregated_chars} chars of context from {added_chunks_count} chunks across {len(context_sources)} files.")
        else:
             logger.warning(f"No context aggregated within limits.")


        logger.info(f"Total aggregated context characters: {total_aggregated_chars}")
        # The structure changed: returns a dict with 'combined_context' and 'source_files'
        return all_context


    def query_llm(self, query, context_data, retry_count=1): # Takes context_data dict
        ...
        if not combined_context:
            logger.warning(f"No context provided for LLM query.")
        ...
        try:
            ...
            system_prompt = f"""
            You are an AI assistant specialized in answering questions based *only* on the provided context derived from one or more source documents ({source_file_str}).
...
- If the context does not contain the information needed to answer the question, you MUST state: "Based on the provided context from {source_file_str}, I cannot answer this question."
- Do NOT use any external knowledge, make assumptions, or infer information not explicitly stated.
- Be concise and directly answer the question.
- If you cannot answer, state which document(s) were consulted based on the provided context excerpts (using the filenames provided in the context markers like 'Source: filename.ext').
"""
...
Answer based *only* on the provided context:"""
            ...
            for attempt in range(retry_count + 1):
                try:
                    ...
                    elif isinstance(response, str): # Fallback
                    ...
                    if not answer:
                         logger.warning(f"Received empty response from Groq on attempt {attempt+1}")
                         answer = f"Received an empty response from the language model."
                         if attempt < retry_count: time.sleep(1); continue
                         else: break # Stop if still empty after retries

                    # Valid answer received (even if it's "cannot answer").
                    return answer

                except Exception as e_api:
                    logger.warning(f"Groq API query attempt {attempt+1} failed: {e_api}")
                    if attempt < retry_count:
                        wait_time = 1.5 ** attempt
                        logger.info(f"Retrying Groq query in {wait_time:.1f}s...")
                        time.sleep(wait_time)
                    else:
                        logger.error(f"All Groq API query attempts failed: {e_api}", exc_info=True)
                        answer = f"Error: Failed to get answer from language model after {retry_count+1} attempts. (API Error: {e_api})"
            ...
            if "streamlit" in sys.modules:
                st.error(f"Failed to query the language model. Error: {e_setup}")
            return f"Error: Could not query language model due to setup error: {e_setup}"
    ...
    def run_query(self, query, context_strategy="top_k"):
        ...
        final_results = {
            "query": query,
            "retrieval_results": {}, # Original per-file results for inspection
            "aggregated_context_data": {}, # Dict containing 'combined_context' and 'source_files'
            "answer": "", # Single unified answer
            "status": "Started",
            "error": None
        }
        ...
        try:
            ...
            # Check if running in Streamlit before using st.spinner
            retrieve_context = {}
            if "streamlit" in sys.modules:
                with st.spinner("Searching relevant documents..."):
                     query_results = self.query_files(query)
            else:
                 query_results = self.query_files(query)
            ...
            # 2. Aggregate context (Now combines results)
            aggregate_context = {}
            if "streamlit" in sys.modules:
                 with st.spinner("Gathering context..."):
                     aggregated_context_data = self.aggregate_context(query_results, strategy=context_strategy)
            else:
                 aggregated_context_data = self.aggregate_context(query_results, strategy=context_strategy)
            ...
            # 3. Query LLM with the single aggregated context
            llm_context = {}
            if "streamlit" in sys.modules:
...
# Need to import sys for checking if running in streamlit
import sys
...
if __name__ == "__main__":
    ...
    def load_rag_system(config_path="config.ini"):
        if not os.getenv("GROQ_API_KEY"):
            ...
            st.stop()
        try:
            # Pass Streamlit context flag or check within init
    ...
    def process_documents(_rag_system):
        ...
        def streamlit_progress_callback(message, is_error=False, is_warning=False, is_done=False, current_step=None, total_steps=None, stage=None):
            log_prefix = ""
            progress_val = 0.0 # Default progress

            if is_error: log_prefix = "❌ Error: "
            elif is_warning: log_prefix = "⚠️ Warning: "
            elif is_done: log_prefix = "✅ "; progress_val = 1.0 # Done means 100%
            elif stage: log_prefix = f"⏳ ({stage}) " # Indicate stage if provided
            ...
            if total_steps and current_step is not None:
                # Estimate progress within a step using stage info (0.5 offset)
                stage_progress = 0.0
                if stage == "Extracting": stage_progress = 0.1
                elif stage == "Chunking": stage_progress = 0.3
                elif stage == "Verifying": stage_progress = 0.5
                elif stage == "Embedding": stage_progress = 0.6
                elif stage == "Indexing": stage_progress = 0.9

                progress_percent = min(1.0, (current_step + stage_progress) / total_steps) if total_steps > 0 else 0
                progress_val = progress_percent
            elif is_done:
                progress_val = 1.0
            ...
            if status_placeholder:
                 try:
                      # Only update progress if value is > 0 or if done
                      if progress_val > 0 or is_done:
        ...
        st.caption(f"Looking for files ({supported_ext_str}) in: `{os.path.abspath(_rag_system.config.data_dir)}`")
        st.caption(f"Storing index files in: `{os.path.abspath(_rag_system.config.index_dir)}`")
        # status_placeholder defined above

        with st.spinner(f"Initializing and processing documents... This may take time."):
            try:
                processing_successful = _rag_system.process_files(progress_callback=streamlit_progress_callback)

                # Final status update logic after processing attempt
                indexed_count = len(_rag_system.faiss_indexes)
                processed_file_names = _rag_system.processed_files # Files that had an index created

                if indexed_count > 0:
                    status_placeholder.success(f"✅ Ready! Processed and indexed {indexed_count} document(s).")
                elif processing_successful and indexed_count == 0 and not _rag_system.processed_files:
                    # Processing technically succeeded but found no supported files or all failed extraction/indexing early
                    status_placeholder.warning(f"⚠️ No supported documents were found or could be indexed in '{_rag_system.config.data_dir}'. Please add files.")
                elif processing_successful and indexed_count == 0 and _rag_system.processed_files:
                     # This case shouldn't really happen if processed_files tracks successful indexing
                     status_placeholder.warning(f"⚠️ Processing finished, but no documents ended up in the index. Check logs.")
                elif not processing_successful: # process_files returned False due to a directory error etc.
                     # The error message should have been set by the callback
                     if not any("❌ Error:" in msg for msg in status_messages): # Add generic error if callback missed it
                         status_placeholder.error(f"❌ Processing failed. Check logs for errors.")
                else: # Fallback for unexpected state
                     status_placeholder.info("Processing status unclear. Check logs.")
        ...
        # Return success status (at least one doc indexed) and list of indexed filenames
        return indexed_count > 0, sorted(list(_rag_system.faiss_indexes.keys()))
    ...
    try:
        ...
        # Use st.rerun() to force reprocessing if button clicked
        if st.button("🔄 Re-process Documents"):
            st.cache_data.clear() # Clear data cache
            st.cache_resource.clear() # Clear resource cache (RAG system itself)
        ...
        if is_ready and indexed_files:
            ...
            if user_query:
                if st.button("Get Answer", key="submit_query"):
                    ...
                    # Removed outer spinner, inner spinners handle stages
                    ...
                    if answer:
                        st.markdown(answer) # Display the single, combined LLM answer
                    else:
                    ...
                    # Display Combined Context & Retrieval Details
                    ...
                    if combined_context:
                         ...
                         st.info("No context was aggregated for the LLM.")

                    # Display Original Retrieval Details (per file)
                             ...
                             for file_name, file_res_list in retrieval_results.items():
                                 if file_res_list:
                                     ...
                                     for i, res in enumerate(file_res_list):
                                         ...
                                         # Display the *actual* content of the chunk retrieved
            ...
            # This state is reached if process_documents returns indexed_count=0
            st.warning(f"No documents are currently indexed or ready for querying.")
            st.info("Please check the 'Document Processing' section above for status or add supported files and click 'Re-process Documents'.")
        else: # Not ready, processing likely failed critically (is_ready=False)
